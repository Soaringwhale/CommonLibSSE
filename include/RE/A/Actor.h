...
namespace RE
{
	...
	public:
		...
		bool                            GetClipInfo(const BSScrapArray<BSFixedString>& events, int32_t activeGraph_ind, AnimationSystemUtils::ClipInfo& clipInfo);
		...
		const NiPointer<BGSAttackData>& GetCurrentAttackData() const;
		TESPackage*                     GetCurrentPackage();
		const TESPackage*               GetCurrentPackage() const;
		bool                            GetCurrentPathingRequest(BSTSmartPointer<BSPathingRequest>& ans) const;
		...
		TESObjectWEAP*                  GetWeapon(bool left);
		...
		bool                            SetPathfindingGoal(const BSTSmartPointer<BSPathingRequest>& goal);
